...
Class CountryDetection{
		...
		public function get_country($code){
			...
			if ( !empty($code) ){
				...
				$output = ucwords(strtolower($countries[strtoupper($code)]));
			}
		return $output;
	}

	public function get_country_code(){
	//https://albertoroura.com/country-detection-via-php/
	   ...
	   $current_sites_list = array(
		...
		curl_setopt( $ch, CURLOPT_URL, 'http://ipinfo.io/' . $visitor_ip . '/country');
		curl_setopt( $ch, CURLOPT_TIMEOUT, 3);
		curl_setopt( $ch, CURLOPT_RETURNTRANSFER, 1 );

		$output			= curl_exec( $ch );
		$country_code 	= trim(strtoupper( $output ));
		
		curl_close( $ch );
		
		//if country is missing or blank then assign a blank value
		if (empty($country_code) || !in_array( $country_code, $current_sites_list ) ){
			$country_code = '';
		}

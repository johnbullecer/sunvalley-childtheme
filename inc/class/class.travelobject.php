...
class TravelerObject
{
    ...
    function init()
    {
         ...
         * @since 1.3.0
         */
        add_filter( 'rewrite_rules_array',array($this, 'st_insert_rewrite_rules' ));
        add_filter( 'query_vars',array($this, 'st_insert_query_vars' ));
        add_action( 'wp_loaded',array($this, 'st_flush_rules' ));

        add_action('pre_get_posts', array( $this, 'change_author_get_posts'), 50);
    ...
    **/
    ...
    public function st_flush_rules(){
        ...
        if ( ! isset( $rules['page-user-setting/([^/]+)?$'] ) || ! isset( $rules['page-user-setting/([^/]+)/page/([0-9]{1,})?$'] )) {
            global $wp_rewrite;
    ...
    function  _class_init()
    {
        ...
        add_filter( 'wp_nav_menu_items', array($this,'st_custom_menu_mobile_item'), 10, 2 );
    ...
    function _top_ajax_search()
    {
        ...
        add_filter( 'pre_get_posts' , array( $this , '_change_top_search' ) );
        ...
        remove_filter( 'pre_get_posts' , array( $this , '_change_top_search' ) );
...
sin( radians( mt1.meta_value ) ) ) ) AS distance
    ...
    function get_near_by_lat_lng($st_location = false ,  $lat = false , $lng = false ,$post_type=array(), $range = 20 , $limit = 5 )
    {
        $map_lat   = (float)$lat;
        $map_lng   = (float)$lng;
        //Search by Kilometer :6371
...
sin( radians( mt1.meta_value ) ) ) ) AS distance
    ...
    static function get_min_max_price( $post_type )
    {
        ...
        switch($post_type){
            case "st_hotel":
                if(STAdminHotel::check_ver_working() == true){
                ...
                break;
            case "hotel_room":
                if(STAdminActivity::check_ver_working() == true){
                    $where = "where 1=1 and ( {$wpdb->posts}.post_status = 'publish' )" ;
                    $where = STLocation::edit_where_wpml($where);
                ...
                break;
            case "st_rental":
                if(STAdminRental::check_ver_working() == true){
                    $where = "where 1=1 and ( {$wpdb->posts}.post_status = 'publish' )" ;
                    $where = STLocation::edit_where_wpml($where);
                    $sql = "SELECT
                ...
                break;
            case "st_cars":
                if(STAdminCars::check_ver_working() == true){
                    $where = "where 1=1 and ( {$wpdb->posts}.post_status = 'publish' )" ;
                    $where = STLocation::edit_where_wpml($where);
                    $sql = "SELECT
                ...
                break;
            case "st_tours":
                if(STAdminTours::check_ver_working() == true){
                    $sql = "
                    select
        ...
        if (empty($price_max)) {$price_max = 500  ;} // default 0 500
        if (empty($price_min)) {$price_min = 0  ;} // default 0
        if ($price_max<= $price_min) {$price_max = 2 * $price_min ;}
    ...
    static function get_list_location(){
        ...
        if (!empty($getListFullNameLocation) and is_array($getListFullNameLocation)) {
            ...
            foreach($getListFullNameLocation as $key=>$value) {
                $return[] = array('title' => $value->post_title,'id'=> $value->ID);
            }
            return $return;
        }
        return false ;
    ...
    */
	...
	static function check_cancel_able($order_id)
	{
		...
		if(!$check_order){
			...
			//commented out this conditional coz its a bug. added by clyde
    ...
    function st_custom_menu_mobile_item ( $items, $args ) {
        if ($args->theme_location == 'primary') {
            ...
            //$html .= st()->load_template('menu/mobile/shopping_cart');
            $sort_topbar_menu = st()->get_option( 'sort_topbar_menu', false );
            if( $sort_topbar_menu ){
                foreach( $sort_topbar_menu as $key => $val ){
                    if( $val['topbar_item'] == 'shopping_cart' ){
                        $html .= st()->load_template("menu/mobile/shopping_cart" , null ,  array('container' =>"li"  , "class"=>"top-user-area-shopping"));
                    }
                }
            }
            $items .= $html;

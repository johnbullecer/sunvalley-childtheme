...
if(!class_exists( 'STUser_f' )) {
    class STUser_f extends TravelerObject
    {
        ...
        function init()
        {
            ...
            add_action('init', array( $this, 'reset_password'), 50 );
        ...
        public function reset_password(){
            if( STInput::post('action','') == 'reset_password' && 
                ...
                $password = esc_html( trim(STInput::post('password', '')));
                $re_password = esc_html( trim(STInput::post('re-password', '')));
                if( strlen( $password ) < 6 ){
                    STTemplate::set_message( __('Your password is invalid. Enter at least 6 characters ', ST_TEXTDOMAIN), 'danger');
                    return false;
                }
                if( $password != $re_password ){
                    STTemplate::set_message( __('Re-password do not match with password', ST_TEXTDOMAIN), 'danger');
                    return false;
                }
                ...
                STTemplate::set_message( __('Your password has been reset.', ST_TEXTDOMAIN), 'success');
                return true;
            }
        }
        ...
        public function st_get_cancel_booking_step_1(){
            ...
            echo json_encode( array(
                'status' => 0,
                'message' => '<div class="text-danger">'. __('Error when retrieving booking data. Please try again!', ST_TEXTDOMAIN) . '</div>',
                'order_id' => $order_id,
                'order_encrypt' => $order_encrypt,
                'step' => ''
            ) ); exit(0);
            ...
            switch($register_as){
                ...
                case "partner":
                    ...
                    if( !$validate ){
                        ...
                        $html = '<div  class="alert alert-danger"><button data-dismiss="alert" type="button" class="close"><span aria-hidden="true">×</span></button>';
                        $html .= "<strong>".__( 'Error!.' , ST_TEXTDOMAIN )."</strong>".__( 'Please upload a image certificate for each service seletected' , ST_TEXTDOMAIN );
                        $html .= '</div>';
                        break;
                    }
                ...
                if(wp_verify_nonce( $_REQUEST[ 'st_insert_post_hotel' ] , 'user_setting' )) {
                    ...
                    if(!empty( $post_id )) {
                        ...
                        if(!empty($_FILES[ 'featured-image' ]['name']) and empty($_REQUEST['id_featured_image'])){
                            ...
                            set_post_thumbnail( $post_id , STInput::request('id_featured_image') );
                        }
                        /////////////////////////////////////
                        /// Update logo
                        /////////////////////////////////////
